...
void anaL2::h2D2Init(TH2D *h2D2[USBWCnChannels],TString h1name, TString h1Title,
  ...
  for(i = 0;i<USBWCnChannels;i++){    
    h1nameh = h1name; h1nameh += "_"; h1nameh += "ch_"; h1nameh += i;
    h1Titleh = h1Title; h1nameh += " "; h1Titleh += "ch "; h1Titleh += i;
    h2D2[i] = new TH2D(h1nameh.Data(), h1Titleh.Data(),
                       Nbin1, Vmin1, Vmax1,
		       Nbin2, Vmin2, Vmax2);
  }
}
...
void anaL2::Loop0(TString nameF){
  cout<<"anaL2::Loop0(TString nameF)"<<endl;
  ...
  h1D1Init(h1_amplMax,"h1_amplMax","amplMax",20000,-0.1,2.5);
  h1D1Init(h1_amplMaxTime,"h1_amplMaxTime","amplMaxTime",800,0.0,2500);
  h1D1Init(h1_amplMin,"h1_amplMin","amplMin",100,-2.5,0.1);
  h1D1Init(h1_amplMinTime,"h1_amplMinTime","amplMinTime",800,0.0,2500);
  h1D1Init(h1_amplBaseline,"h1_amplBaseline","amplBaseline",100,-2.5,2.5);
  h1D1Init(h1_timeCFD,"h1_timeCFD","timeCFD",40000,0.0,2500);
  h1D1Init(h1_deltaTimeCFD,"h1_deltaTimeCFD","delta TimeCFD",4000,-100.0,100.0);
  h1D1Init(h1_timeConstTh,"h1_timeConstTh","timeConstTh",800,0.0,2500);
  h1D1Init(h1_sigRiseT,"h1_sigRiseT","sigRiseT",1000,0.0,20);
  h1D1Init(h1_sigWidth,"h1_sigWidth","sigWidth",4000,0.0,300);
  h1D1Init(h1_sigFallT,"h1_sigFallT","sigFallT",4000,0.0,300);
  h1D1Init(h1_sigChargeInWindowVT,"h1_sigChargeInWindowVT","sigChargeInWindowVT",10000,-1,30);
  h1D1Init(h1_sigChargeInWindowVT_npe,"h1_sigChargeInWindowVT_npe","sigChargeInWindowVT npe",200,-1,30);
  ...
  TH1D *h1_sigChargeInWindowVT_cut[USBWCnChannels];
  TH1D *h1_deltaTimeCFD_cut[USBWCnChannels];
  h1D1Init(h1_amplMax_cut,"h1_amplMax_cut","amplMax cut",20000,-0.1,2.5);
  ...
  h1D1Init(h1_sigRiseT_cut,"h1_sigRiseT_cut","sigRiseT cut",1000,0.0,20);
  h1D1Init(h1_sigWidth_cut,"h1_sigWidth_cut","sigWidth cut",4000,0.0,300);
  h1D1Init(h1_sigFallT_cut,"h1_sigFallT_cut","sigFallT cut",4000,0.0,300);
  h1D1Init(h1_sigChargeInWindowVT_cut,"h1_sigChargeInWindowVT_cut","sigChargeInWindowVT_cut",10000,-1,20);
  h1D1Init(h1_deltaTimeCFD_cut,"h1_deltaTimeCFD_cut","delta TimeCFD cut",4000,-100.0,100.0);
  //
  TH2D *h2_sigRiseT_vs_amplMax[USBWCnChannels];
  h2D2Init(h2_sigRiseT_vs_amplMax, "h2_sigRiseT_vs_amplMax", "sigRiseT vs amplMax",
		400, -0.01, 0.8,
		400,   0.0, 10.0);
  //
  TH2D *h2_sigRiseT_vs_amplMax_cut[USBWCnChannels];
  h2D2Init(h2_sigRiseT_vs_amplMax_cut, "h2_sigRiseT_vs_amplMax_cut", "sigRiseT vs amplMax cut",
		400, -0.01, 0.8,
		400,   0.0, 10.0);
  //
  ...
  //
  TH2D *h2_amplMax_ch0_vs_ch2 = new TH2D("h2_amplMax_ch0_vs_ch2","h2_amplMax_ch0_vs_ch2",800,-0.1,2.5,800,-0.1,2.5);
  TH2D *h2_amplMax_cut_ch0_vs_ch2 = new TH2D("h2_amplMax_cut_ch0_vs_ch2","h2_amplMax_cut_ch0_vs_ch2",800,-0.1,2.5,800,-0.1,2.5);
  //
  Long64_t nentries = fChain->GetEntriesFast();
  cout<<"nentries = "<<nentries<<endl;
  cout<<setw(10)<<"ientry"<<setw(20)<<"amplMax[i]"<<setw(20)<<"timeCFD[i]"<<setw(20)<<"sigFallT[i]"<<endl;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for(i = 0;i<USBWCnChannels;i++){
      ...
      h1_deltaTimeCFD[i]->Fill(timeCFD[i] - timeCFD[0]);
      ...
      //
      h2_sigRiseT_vs_amplMax[i]->Fill(amplMax[i],sigRiseT[i]);
      //
      if( amplMax[0] > 0.15){
	if( amplMax[0] < 0.25){
	  if( amplMax[2] > 0.45){
	    if( amplMax[2] < 0.6){
	      if( (timeCFD[2] - timeCFD[0]) > ( 9.358 - 0.25*3 )){
		if( (timeCFD[2] - timeCFD[0]) < ( 9.358 + 0.25*3 )){
		  if( (timeCFD[1] - timeCFD[0]) > (12.51 - 0.25*3 )){
		    if( (timeCFD[1] - timeCFD[0]) < ( 12.51 + 0.25*3 )){		      
		      //if( amplMax[1] > 0.006){
			...
			h1_sigChargeInWindowVT_cut[i]->Fill(sigChargeInWindowVT[i]);
			h1_deltaTimeCFD_cut[i]->Fill(timeCFD[i] - timeCFD[0]);
			//
			h2_sigRiseT_vs_amplMax_cut[i]->Fill(amplMax[i],sigRiseT[i]);
			//
			if(i == 1)
			  cout<<setw(10)<<ientry<<setw(20)<<amplMax[i]<<setw(20)<<timeCFD[i]<<setw(20)<<sigFallT[i]<<endl;
			//
			if(i == 0){
			  h2_amplMax_cut_ch0_vs_ch2->Fill(amplMax[2],amplMax[0]);
			}
			//}
		    }
		  }
    ...
    h2_amplMax_ch0_vs_ch2->Fill(amplMax[2],amplMax[0]);
    //if(sigFallT[0]>1800)
  ...
  for(i = 0;i<USBWCnChannels;i++){
    ...
    //
    h2_sigRiseT_vs_amplMax[i]->Write();
    //
    ...
    h1_sigChargeInWindowVT_cut[i]->Write();
    h1_deltaTimeCFD_cut[i]->Write();
    //
    h2_sigRiseT_vs_amplMax_cut[i]->Write();
  }
  h2_amplMax_ch0_vs_ch1->Write();
  h2_amplMax_ch0_vs_dUT->Write();
  h2_amplMax_ch1_vs_dUT->Write();
  //
  h2_amplMax_ch0_vs_ch2->Write();
  h2_amplMax_cut_ch0_vs_ch2->Write();
  //
  cout<<endl<<endl;
  rootFile->Close();
}
